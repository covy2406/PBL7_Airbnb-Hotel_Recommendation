...
const Search = () => {
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-12 pt-12 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {data.map((item) => (
          <Link component={Link} to={`/hotels/${item.id}`} className="cursor-pointer">
            ...
            <div className="flex items-start justify-between pt-3">
              {/* Left */}
              <div className="">
                <h5 className="max-w-[17rem] text-[17px] font-medium -mt-1 text-black-500">{item.name}</h5>
